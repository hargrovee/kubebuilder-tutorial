...
func (r *CronJobReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if cronJob.Spec.ConcurrentPolicy == batchv1.ForbidConcurrent && len(activeJobs) > 0 {
		log.V(1).Info("concurrency policy blocks concurrent runs, skipping",
			"num active", len(activeJobs))
		return scheduledResult, nil
	}

	// ...or instruct us to replace existing ones...
	if cronJob.Spec.ConcurrentPolicy == batchv1.ReplaceConcurrent {
		for _, activeJob := range activeJobs {
	...
	constructJobForCronJob := func(cronJob *batchv1.CronJob, scheduledTime time.Time) (*kbatch.Job,
		error) {
		//	We want job names for a given nomial start time to have a deteministic name
		//	to aovid the same job being created twice
...
func (r *CronJobReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &kbatch.Job{},
